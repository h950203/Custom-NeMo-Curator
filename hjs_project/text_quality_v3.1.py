...
INPUT_PATH = "data/"
OUTPUT_PATH = "filtered_data_v3.1/"
REMOVED_LOGS_PATH = "filtered_data_v3.1_log/"
...
def process_file_with_tracking(input_file, output_dir, config):
    ...
    try:
        ...
        # 'process_and_score_fields'와 'run_filters_on_text' 함수를 직접 가져와서 사용
        from nemo_curator.stages.text.quality.utils import run_filters_on_text, calculate_scores
...
def save_preprocessing_logs(logs_dir):
    """전처리 과정에서 변경된 내용을 로그 파일로 저장"""
    os.makedirs(logs_dir, exist_ok=True)
...
def main():
    ...
    try:
        ...
        save_preprocessing_logs(REMOVED_LOGS_PATH)
