...
def main():
    ...
    pipeline = Pipeline(
        name="quality_filtering_korean_v2.2",
    ...
    output_path = "filtered_data_v2.2/"
    ...
    def split_document_into_sentences(doc):
        doc["sentences"] = kss.split_sentences(doc)
        return doc
    pipeline.add_stage(Modify(modifier_fn=split_document_into_sentences))

    # STAGE 1B: Per-Sentence Preprocessing
    personal_preprocessor = PersonalFilter()
    def process_each_sentence(doc):
        if "sentences" in doc:
            processed_sentences = [personal_preprocessor.apply(s) for s in doc["sentences"]]
            doc["sentences"] = [s for s in processed_sentences if s]  # Remove empty sentences
        return doc
    pipeline.add_stage(Modify(modifier_fn=process_each_sentence))

    # STAGE 1C: Sentence Joining
    def join_sentences_into_document(doc):
        if "sentences" in doc:
            doc["text"] = " ".join(doc["sentences"])
            del doc["sentences"]
        return doc
    pipeline.add_stage(Modify(modifier_fn=join_sentences_into_document))
