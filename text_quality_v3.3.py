...
# Ray 설정
RAY_TEMP_DIR = "/data/ray_tmp"  # /tmp 대신 큰 디스크 경로로 변경하세요
OBJECT_STORE_MEMORY = 20 * 1024**3  # 20GB (시스템 RAM에 맞게 조정)
MAX_CONCURRENT_FILES = 4  # 동시 처리 파일 수 제한
...
def main():
    ...
    try:
        ...
        num_cpus = max(4, cpu_count() // 2)  # CPU 수 절반으로 제한
        
        print(f"Ray 임시 디렉토리: {RAY_TEMP_DIR}")
        print(f"Object Store 메모리: {OBJECT_STORE_MEMORY / 1024**3:.1f} GB")
        print(f"사용 CPU 수: {num_cpus}/{cpu_count()}")
        ...
        print(f"동시 처리 파일 수: {MAX_CONCURRENT_FILES}")
        print("-" * 70)
        ...
        # 파일 크기순 정렬 (작은 파일부터 처리)
        input_files_with_size = [(f, os.path.getsize(f)) for f in input_files]
        input_files_with_size.sort(key=lambda x: x[1])
        input_files_sorted = [f for f, _ in input_files_with_size]
        
        print(f"\n발견된 파일: {len(input_files_sorted)}개")
        ...
        print(f"\n{len(input_files_sorted)}개의 파일을 {MAX_CONCURRENT_FILES}개씩 배치 처리합니다...")
        
        total_processed = 0
        with tqdm(total=len(input_files_sorted), desc="Total Progress") as pbar:
            for i in range(0, len(input_files_sorted), MAX_CONCURRENT_FILES):
                batch = input_files_sorted[i:i+MAX_CONCURRENT_FILES]
                print(f"\n배치 {i//MAX_CONCURRENT_FILES + 1}: {len(batch)}개 파일 처리 중...")
                
                # 배치 내 파일들을 병렬 처리
                batch_futures = [process_file_remote.remote(f) for f in batch]
                ...
                for future in batch_futures:
                    try:
                        ...
                        print(f"\n✗ Ray future 결과 처리 중 오류: {e}")
                        traceback.print_exc()
                        pbar.update(1)
                
                # 배치 처리 후 메모리 정리
                print(f"배치 완료. 메모리 정리 중...")
                gc.collect()
                
                # 진행 상황 출력
                print(f"진행률: {total_processed}/{len(input_files_sorted)} ({total_processed/len(input_files_sorted)*100:.1f}%)")
